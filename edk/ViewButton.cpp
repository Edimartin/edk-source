...
//remove callback
void edk::ViewButton::removeCallback(){
    //
}
...
void edk::ViewButton::deleteSpritePressed(){
...
//delete the spriteUp
void edk::ViewButton::deleteSpriteUp(){
    //test if have the sprite
...
bool edk::ViewButton::loadFontImageFromMemory(const char* name,
    ...
    return this->loadFontImageFromMemory((edk::char8*) name,
                                         image,
                                         size,
                                         filter,
                                         color);
...
void edk::ViewButton::update(edk::WindowEvents* events){
    ...
    if(events){
        ...
        if(!this->mouseInside){
            //test if release somre button
            if(events->mouseRelease.size()){
                ...
                for(edk::uint32 i=0u;i<events->mouseRelease.size();i++){
                    ...
                    if(this->holdButton.haveElement(events->mouseRelease[i])){
                        ...
                        this->eventMouseReleased(edk::vec2f32(-1,-1),events->mouseRelease[i]);
                    }
                }
...
void edk::ViewButton::drawPolygon(rectf32 outsideViewOrigin){
    ...
    case edk::buttonView::state::pressed:
